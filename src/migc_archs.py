# Copied from original MIGC implementation
# https://github.com/limuloo/MIGC/blob/main/migc/migc_arch.py
...
class FourierEmbedder:
    def __init__(self, num_freqs: int = 64, temperature: int = 100):
...
class PositionNet(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, fourier_freqs: int = 8):
        super().__init__()
        self.in_dim = in_dim
...
class SAC(nn.Module):
    def __init__(self, C: int, number_pro: int = 30):
        super().__init__()
        self.C = C
    ...
    def forward(
        self, x: torch.Tensor, guidance_mask: torch.Tensor, sac_scale: torch.Tensor | None = None
        ...
        guidance_mask = guidance_mask.view(guidance_mask.shape[0], phase_num, -1)[..., None]  # (B, phase_num, HW, 1)

        null_x = torch.zeros_like(x[:, [0], ...]).to(x.device, dtype=x.dtype)
        null_mask = torch.zeros_like(guidance_mask[:, [0], ...]).to(guidance_mask.device, dtype=guidance_mask.dtype)
        ...
        out = (x * scale).sum(dim=1, keepdim=True)  # (B, 1, HW, C)
        return out, scale


class MIGC(nn.Module):
    def __init__(self, C: int, attn_type: str = "base", context_dim: int = 768, heads: int = 8):
        super().__init__()
        self.ea = CrossAttention(query_dim=C, context_dim=context_dim, heads=heads, dim_head=C // heads, dropout=0.0)
        self.la = LayoutAttention(query_dim=C, heads=heads, dim_head=C // heads, dropout=0.0)
        self.norm = nn.LayerNorm(C)
        self.sac = SAC(C)
        self.pos_net = PositionNet(in_dim=768, out_dim=768)

    def forward(
        self, ca_x: torch.Tensor, guidance_mask: torch.Tensor, other_info: dict, return_fuser_info: bool = False
        ...
        guidance_mask = guidance_mask.to(dtype=ca_x.dtype)
        ...
        supplement_mask = supplement_mask.to(dtype=ca_x.dtype)
        ...
        ea_x, ea_attn = self.ea(
            self.norm(image_token[:, 1:, ...].view(B * instance_num, HW, C)), context=context, return_attn=True
        )
        ...
        fusion_template = self.la(
            x=ori_image_token, guidance_mask=torch.cat([guidance_mask[:, :, ...], supplement_mask], dim=1)
        )  # (B, HW, C)
        ...
        guidance_mask = torch.cat(
            [
                supplement_mask,
                guidance_mask,
                torch.ones(B, 1, H, W, device=guidance_mask.device, dtype=guidance_mask.dtype),
            ],
            dim=1,
        )
...
class NaiveFuser(nn.Module):
    ...
    def forward(
        ...
        guidance_mask = F.interpolate(guidance_mask, size=(H, W), mode="bilinear")  # (B, instance_num, H, W)
        guidance_mask = torch.cat(
            [torch.ones(B, 1, H, W, device=guidance_mask.device, dtype=guidance_mask.dtype), guidance_mask * 10], dim=1
        )  # (B, instance_num+1, H, W)
